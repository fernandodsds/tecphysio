...
var cards = []
var sizeWidth = out.offsetWidth;
var sizeHeight = out.offsetHeight;
var startTime=new Date();
var cursorPos = {x:0, y:0};
var gameInitialized = false;
var gameReadyGo = false
var revealTime = 0;
var gameScore = 0 
var qtdeJogos = 0
...
function onButtonCursor(buttonPos, cursorPos){
  //console.log(cardPos)
  //console.log(cursorPos)
  if (cursorPos.x >= buttonPos.x && cursorPos.x <= (buttonPos.x + buttonPos.width) && cursorPos.y >= buttonPos.y && cursorPos.y <= (buttonPos.y + buttonPos.height)) {
    return true
  } else if(cursorPos.x >= (buttonPos.x + buttonPos.width) && cursorPos.x <= buttonPos.x && cursorPos.y  >= (buttonPos.y + buttonPos.height) && cursorPos.y  <= buttonPos.y){
    return true
  }
...
function drawMessage(ctx, text){
  ...
  boxHeight = 100
  ctx.fillStyle = 'white';
  ctx.rect((out.width/2)-(boxWidth/2), (out.height*0.4 -(boxHeight/2)) , boxWidth, boxHeight);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ctx.font = "12px Arial";
  ...
  ctx.fillStyle = 'black';
  ctx.fillText(text , (out.width/2) - (textWidth / 2), (out.height*0.4))  
  console.log(textWidth)
  ctx.fill();
  ctx.stroke();
}

function drawConfirmMessage(ctx){
  drawMessage(ctx,"Teste");
  btnWidth = 50
  btnHeight = 20
  btn1 = {
    "x":(out.width/2)-(btnWidth*1.5),
    "y": (out.height*0.46 -(btnHeight/2)),
    "width":btnWidth,
    "height": btnHeight
  }
  
  btn2 = {
    "x":(out.width/2)-(btnWidth*-0.6),
    "y": (out.height*0.46 -(btnHeight/2)),
    "width":btnWidth,
    "height": btnHeight
  }  
  ...
  if (onButtonCursor(btn1, cursorPos))
  {
    ctx.fillText('Colision', cursorPos.x, cursorPos.y)    
  };

  if (onButtonCursor(btn2, cursorPos)){
    ctx.fillText('Colision', cursorPos.x, cursorPos.y)    
  };
}
...
function memoryGame(){
  ...
  for (var i = 0; i <6 ;i++){
    drawImageCard( cards[i].image,canvasCtx,cards[i].x,cards[i].y);  
  }  
  ...
  for (var i = 0; i<6;i++){
    if (onCardCursor(cards[i], cursorPos) 
      ...
      canvasCtx.fillText(cards[i].elapsed, cursorPos.x, cursorPos.y)    
...
function onResultsPose(results) {
  ...
  if (gameReadyGo == false){
    drawConfirmMessage(canvasCtx);
  }else{
    memoryGame();
  }
...
cursorPos.x = results.poseLandmarks[19].x*out.width;
cursorPos.y = results.poseLandmarks[19].y*out.height;
